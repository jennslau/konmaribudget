...
class PeacefulFinanceDashboard:
    ...
    def _clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Clean amounts - enhanced for bank statement formats
        if 'amount' in df.columns:
            # Handle various amount formats from bank statements
            df['amount'] = df['amount'].astype(str)
            
            # Remove common formatting characters but preserve negative signs
...
if __name__ == "__main__":
    main()
, r'-\1', regex=True)  # Convert (123.45) to -123.45
            
            # Convert to numeric
            df['amount'] = pd.to_numeric(df['amount'], errors='coerce')
            
            # Handle cases where debits are in separate column but marked differently
            # Some bank statements use positive values for debits with description indicators
            if 'description' in df.columns:
                # Look for withdrawal indicators in description
                withdrawal_indicators = ['wd', 'withdrawal', 'pos wd', 'external wd', 'debit', 'payment', 'fee']
                deposit_indicators = ['deposit', 'dp', 'credit', 'transfer']
                ...
                for idx, row in df.iterrows():
                    if pd.notna(row['amount']) and pd.notna(row['description']):
                        ...
                        # If amount is positive but description indicates it's a withdrawal, make it negative
                        if amount > 0 and any(indicator in desc_lower for indicator in withdrawal_indicators):
                            df.at[idx, 'amount'] = -amount
                        # If amount is negative but description indicates it's a deposit, make it positive
                        elif amount < 0 and any(indicator in desc_lower for indicator in deposit_indicators):
